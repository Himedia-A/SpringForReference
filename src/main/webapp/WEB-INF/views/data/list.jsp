	...
	<ul class="common">
		...
		<li>
			<select name="viewType" class="w-px100" onchange="$('form').submit()">
				...
				<option value="grid" >바둑판 형태</option>
			</select>
		</li>
		<li class="list-view">
	...
	$.ajax({
		...
		/*	console.log(data) //한글이 깨지는 현상 발생해서 commonservice와 컨트롤러에서 utf-8로 인코딩해줘야함
			...
			var tag ="<table class ='pharmacy'>"
				+ '<tr><th class="w-px200">약국명</th><th class="w-pw140">전화번호</th><th>주소</th></tr>';
...
}).on("click", ".map", function(){ // 이런 형태로 작성해야 한다.
	if( $(this).data('y') == 'underfined' || $(this).data('x') == 'underfined'){
		alert("위경도가 지원되지 않아 지도에 표시할수 없습니다!!");
		return;
	}
